#!/usr/bin/env python3
"""Part 4: restore lost p1b+p2 patch keys (cumulative-overwrite bug) and
add translations for Section 5 strategy paragraphs, Section 11 flow steps,
and Section 12 parameters table. Merges with whatever is currently in the
patch block instead of overwriting it."""
import json, re
...
print(f"Existing patch keys per lang (p3): {len(existing_patch['ko'])}")

# ── restored from fix_guide_p1b.py (lost) ────────────────────────────────
s08_callout = {
...
s01_f_descs = {
    ...
    'tr': [
        ...
        'Bugünün gerçekleşen kaybı zaten belirlenen limiti aşmışsa, bot derhal durur ve uyarı gönderir.',
        ...
        'Her tik: fiyatın TP\'ye ulaşıp ulaşmadığını (→ %50 kısmi kapatma, SL başabaşa taşınır) veya SL\'ye ulaşıp ulaşmadığını (→ tam kapatma) kontrol eder.',
    ],
    'es': [
...
restored_p1b = {lang: {} for lang in LANGS}
for lang in LANGS:
    restored_p1b[lang]['s08.callout'] = s08_callout[lang]
    for i, desc in enumerate(s01_f_descs[lang], 1):
        restored_p1b[lang][f's01.f{i}.desc'] = desc

print(f"Restored p1b keys per lang: {len(restored_p1b['ko'])}")

with open('/home/user/oracletrading/_p4_part1.json', 'w', encoding='utf-8') as f:
    json.dump({'existing': existing_patch, 'restored_p1b': restored_p1b}, f, ensure_ascii=False)
print("Saved intermediate state to _p4_part1.json")
