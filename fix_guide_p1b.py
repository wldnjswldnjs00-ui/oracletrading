#!/usr/bin/env python3
"""Fix p1 scoping bug: inject patch INSIDE the IIFE (not a separate script block)"""
import json, re

with open('/home/user/oracletrading/ayilon/bot-guide.html', 'r', encoding='utf-8') as f:
    content = f.read()

# ── Remove the GT_PATCH script block injected by part 1 ─────────────────────
# It starts with '\n<script>\n(function () {\n  var GT_PATCH'
# and ends with '})();\n</script>\n'
patch_block_start = '\n<script>\n(function () {\n  var GT_PATCH'
patch_block_end   = '})();\n</script>\n'

start_idx = content.find(patch_block_start)
if start_idx != -1:
    end_idx = content.find(patch_block_end, start_idx) + len(patch_block_end)
    content = content[:start_idx] + content[end_idx:]
    print("Removed old GT_PATCH script block")
else:
    print("No old GT_PATCH found (ok if first run)")

# ── Build patch dict (same data as before) ───────────────────────────────────
s08_vals = {
...
for lang in ['ko','ja','zh','vi','ru','tr','es','pt','id']:
    ...
    for i, desc in enumerate(f_descs[lang], 1):
        patch[lang][f's01.f{i}.desc'] = desc

patch_json = json.dumps(patch, ensure_ascii=False)

# ── Inject patch INSIDE the IIFE, right after the GT object closes ───────────
# Find the boundary: "    }\n  };\n\n  function captureOriginals()"
TARGET = '    }\n  };\n\n  function captureOriginals()'
assert TARGET in content, "Target boundary not found in GT object!"
...
print("Part 1b done.")
print("  - Removed old GT_PATCH script block")
print("  - Injected i18n patch INSIDE the IIFE (correct scoping)")
print(f"  - Patch has {sum(len(v) for v in patch.values())} keys across {len(patch)} languages")
